...
description: "Use when the task needs a persistent Playwright session for iterative local web or Electron debugging, repeated reloads, and functional plus visual QA through a persistent JavaScript REPL."
---
...
- In Codex, that capability is `js_repl`. If `js_repl` is missing, enable it in `~/.codex/config.toml`:
...
- You can also start a new Codex session with `--enable js_repl` (equivalent to `-c features.js_repl=true`).
- In other agents such as Claude Code, only use this skill if the current runtime exposes an equivalent persistent JavaScript REPL. If it does not, fall back to `playwright`.
- After enabling the feature in your current agent, start a new session so the tool list refreshes.
- Run this workflow with sandboxing disabled when your current agent/runtime requires it for Playwright browser control.
- Run setup from the same project directory you need to debug.
- Treat your REPL reset action (for example `js_repl_reset` in Codex) as a recovery tool, not routine cleanup. Resetting the kernel destroys your Playwright handles.

Examples below use Codex `js_repl` terminology and `codex.emitImage(...)` where applicable. Adapt those calls to the equivalent primitives in your current agent, or skip the image-emission steps when no equivalent helper exists.
